...
import com.nageoffer.ai.ragent.rag.core.vector.MilvusCollectionUpgrader;
...
public class MilvusConfig {
    ...
    private final MilvusCollectionUpgrader collectionUpgrader;
    ...
    /**
     * 应用启动时自动升级 Collection 以支持混合检索
     * <p>
     * 如果配置中启用了混合检索（hybrid-search.enabled=true），
     * 则自动检查并升级 Collection Schema，添加 text 字段以支持 BM25 全文检索
     */
    @PostConstruct
    public void init() {
        log.info("初始化 Milvus 配置...");

        if (Boolean.TRUE.equals(ragDefaultProperties.getHybridSearchEnabled())) {
            String collectionName = ragDefaultProperties.getCollectionName();
            log.info("检测到混合检索已启用，开始升级 Collection: {}", collectionName);

            try {
                collectionUpgrader.upgradeCollectionForHybridSearch(collectionName);
                log.info("Collection 升级完成，已支持混合检索（向量 + BM25）");
            } catch (Exception e) {
                log.error("Collection 升级失败，将回退到纯向量检索模式: {}", e.getMessage(), e);
            }
        } else {
            log.info("混合检索未启用，跳过 Collection 升级");
        }
    }
}
