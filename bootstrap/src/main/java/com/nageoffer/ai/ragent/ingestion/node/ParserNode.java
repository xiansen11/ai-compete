...
import com.nageoffer.ai.ragent.ingestion.util.ImageTypeDetector;
...
import com.nageoffer.ai.ragent.core.parser.OcrDocumentParser;
...
public class ParserNode implements IngestionNode {
    ...
    public NodeResult execute(IngestionContext context, NodeConfig config) {
        ...
        // 智能选择解析器：图片文件使用 OCR，其他文件使用 Tika
        DocumentParser parser = selectParser(mimeType, fileName);
        if (parser == null) {
            return NodeResult.fail(new ClientException("未配置合适的解析器"));
        }
    ...
    private DocumentParser selectParser(String mimeType, String fileName) {
        ...
        if (isImage) {
            log.debug("检测到图片文件，使用 OCR 解析器 - MIME: {}, 文件名：{}", mimeType, fileName);
        ...
        log.debug("使用 Tika 解析器 - MIME: {}, 文件名：{}", mimeType, fileName);
