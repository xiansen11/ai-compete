    ...
    public List<List<Float>> embedBatch(List<String> texts, ModelTarget target) {
        ...
        for (int i = 0, n = texts.size(); i < n; i += maxBatch) {
            ...
            try {
                ...
                log.error("{} embeddings request failed", provider(), e);
                throw new ModelClientException(
                        provider() + " embedding request failed: " + e.getMessage(),
        ...
        for (int i = 0; i < results.size(); i++) {
            if (results.get(i) == null) {
                throw new ModelClientException(
                        "Embedding result missing, index=" + i,
    ...
    private List<List<Float>> doEmbedOnce(List<String> slice, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new ModelClientException(
                        "Calling BaiLian Embedding failed: HTTP " + response.code() + " - " + errBody,
        ...
        if (root.has("error")) {
            ...
            throw new ModelClientException(
                    provider() + " embedding error: " + code + " - " + msg,
                    ModelClientErrorType.PROVIDER_ERROR,
                    null
            );
        }
        ...
        if (data == null) {
            throw new ModelClientException(
                    provider() + " embedding response missing data array",
                    ModelClientErrorType.INVALID_RESPONSE,
                    null
            );
        }
        ...
        for (JsonElement el : data) {
            ...
            if (emb == null) {
                throw new ModelClientException(
                        provider() + " embedding response missing embedding field",
                        ModelClientErrorType.INVALID_RESPONSE,
                        null
                );
            }
